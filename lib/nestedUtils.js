...
// 当前元素的classKey
export const ITEM_CLASS_KEY = "itemUniqueClassName";
 ...
 * 更新移动拖拽信息  key
 * itemId  当前拖动元素的id
 * itemUniqueClassName 当前拖动元素的唯一类名
 * isNew 是否是新建的元素
 ...
 * @param {*} layoutUniqueClassName
 */
export function isSelfOrParentLayoutDragging(
  layoutUniqueClassName: String
): boolean {
  let parentDragging = false;
  const layoutEle = document.getElementsByClassName(layoutUniqueClassName)?.[0];
  if (layoutEle) {
    const itemUniqueClassName = getMoveDraggingField(ITEM_CLASS_KEY);
    let parentEle = layoutEle;
    while (parentEle) {
      const parentClassName = parentEle?.className || "";
      if (parentClassName?.indexOf(itemUniqueClassName) >= 0) {
        parentDragging = true;
        break;
      }
